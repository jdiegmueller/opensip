...
[![Status](https://img.shields.io/badge/status-alpha-orange)](#bilinen-sınırlamalar)
...
> ⚠️ **Erken alfa.** Gerçek bir Türk bulut-PBX provider'ı (netsantral.com) üzerinden uçtan uca **iki yönlü ses** ile doğrulandı. Yine de RFC 3261'in birkaç önemli parçası eksik — production'a almadan önce [Bilinen sınırlamalar](#bilinen-sınırlamalar) bölümünü okuyun.
...
`opensip` minimal-viable bir UA: gerçek bir provider ile çağrı yapar ama RFC 3261'in birkaç önemli parçası **henüz yok**. Telefon altyapısı kurmadan önce farkında olun.
...
- **Faz 1 — sağlamlaştırma:** transaction katmanı (RFC 3261 §17), tam dialog state machine, rport/received NAT, TCP transport, RTCP SR/RR
